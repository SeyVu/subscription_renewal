#########################################################################################################
#  Description: Create synthetic data to add to original dataset
...
# Adding synthetic data to increase data size - Create temporary df and then randomly modify values
# (between 80% to 120% of original value) in there and concat to churn_df.
# Need to be run only once to save time, then new df will be saved to file data/data_synthetic.csv
def create_synthetic_data_offsets_from_original():
    ...
    churn_df.to_csv('data/data_synthetic.csv', sep=',')

    logger.debug(churn_df)


# Adding synthetic data to increase data size - use mean and standard deviation from each feature's original data
# to create more artificial data. Need to be run only once to save time, then new df will be saved to
# file data/data_synthetic.csv
def create_synthetic_data_gaussian():
    ...
    for column_name in charge_list:
        ...
        # Multiply # of mins with charge_per_min_mean from original data. Need to make sure this data has a really
    ...
    # Compare original and synthetic data stats
    logger.debug("\noriginal data mean\n")
